			...
			if (Yuntai_tingzhi_flag == 1)//停止指令
			{
				...
				if ((fw_reset_flag == 1) && (fy_reset_flag == 1))//判断当前不是复位运动中,否则不允许手动停止
				{
					...
					if (fy_ms_count_temp > 0)
					{
						...
						fy_cos_value = (double)(((282.8784 * 282.8784) + (219.7559 * 219.7559) - 57.5 * 57.5 - (99.725 + fy_currentPos) * (99.725 + fy_currentPos)) / (2 * 282.8784 * 219.7559));
					
						gul_jiaodu = acos(fy_cos_value) * 180 / 3.14159265  - 25.66769;
...
void jiaodu_send()
{
	...
	gul_fuyang_jiaodu = (short)(acos(fy_cos_value) * 18000 / 3.14159265 - 2566.769);//俯仰角度
...
void AT24c256_dta_Anal()
{
	...
	if (flag == 2)
	{
		...
		fuyang_TargettPos_temp = (double)(sqrt((282.8784 * 282.8784) + (219.7559 * 219.7559) - (2 * fuyang_TargettPos_temp * 282.8785 * 219.7559) - 57.5 * 57.5) - 99.725);
